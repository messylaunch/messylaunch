...
- **Business** belongs to a Client and doubles as the public portfolio entry (`isPublished`, plus `founderName`/`founderPhotoUrl`/`founderBio` — the shoutout for the person/team behind it). Manage in `/admin/businesses` (list + per-business edit + publish/unpublish toggle — new content always starts as an unpublished draft). Surfaces publicly on the home page (niche browser + trusted-by strip), `/work`, and `/niches/[slug]`.
- **Home page** (`app/page.tsx` + `components/FilmHero.tsx`) is a scroll-film: one continuous SVG line scrubbed by scroll (Lenis + manual rAF playhead, no ScrollTrigger). It implements the dev contract — `/?jump=<scrollY>` lands pre-settled and `window.__ready` gates screenshots. The page is locked to the brand cream-paper light theme via a `data-theme="light"` wrapper (logo cream/orange/blue, splat + marker linework in `components/Logo.tsx`). Public intake: `/start` → `POST /api/leads` (honeypot-protected) → Lead row + admin notification → `/admin/leads`.
